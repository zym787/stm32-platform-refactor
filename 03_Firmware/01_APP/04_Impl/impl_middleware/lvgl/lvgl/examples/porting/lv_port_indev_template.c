...
extern timebase_interface_t g_timebase_instance;
extern yield_interface_t g_yield_instance;
extern iic_driver_interface_t g_iic_driver_instance;
extern gpio_opt_interface_t gpio_opt_instance;
...
static bool touchpad_is_pressed(void)
{
    /*Your code comes here*/
    uint8_t figer_num;
    uint8_t rt = cst816t_driver_instance.pf_cst816_get_finger_num(&cst816t_driver_instance, &figer_num);
    if (rt != 0) { // 如果有错误的话
    }
    if(figer_num!=0){
    return true;
