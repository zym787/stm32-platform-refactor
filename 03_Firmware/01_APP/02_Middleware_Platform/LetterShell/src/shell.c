...
signed char shellCheckPermission(Shell *shell, ShellCommand *command)
{
    return ((!command->attr.attrs.permission ||
             ...
             (shell->info.user && (command->attr.attrs.permission &
...
void shellInsertByte(Shell *shell, char data)
{
    ...
    else if (shell->parser.cursor < shell->parser.length)
    {
        ...
        {
            shellWriteByte(
                shell, shell->status.isChecked ? shell->parser.buffer[i] : '*');
        }
        for (short i = shell->parser.length - shell->parser.cursor; i > 0; i--)
        {
 ...
 */
int shellSplit(char *string, unsigned short strLen, char *array[],
               char splitKey, short maxNum)
{
...
int shellGetVarValue(Shell *shell, ShellCommand *command)
{
    ...
    case SHELL_TYPE_VAR_NODE:
    {
        ...
        value =
            func ? func(((ShellNodeVarAttr *)command->data.var.value)->var) : 0;
        break;
    }
    default:
        break;
                ...
                if (((ShellNodeVarAttr *)command->data.var.value)->var)
                {
                    int (*func)(void *, int) =
                        (int (*)(void *, int))(
                            ...
                            ->set;
                    func(((ShellNodeVarAttr *)command->data.var.value)->var,
                         value);
                }
                else
                {
                    int (*func)(int) = (int (*)(
                        int))((ShellNodeVarAttr *)command->data.var.value)
                                           ->set;
                    func(value);
                }
            }
...
{
    ShellCommand *command =
        shellSeekCommand(shell, cmd, shell->commandList.base, 0);
    if (command)
    {
